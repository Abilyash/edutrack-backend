...
export default function CoursePage() {
  ...
  const gradedScores: Record<string, number> = {}
  if (isStudent) {
    allTopics.forEach(t => {
      const subs = submissionsMap[t.id] ?? []
      const scores = subs.filter(s => s.grade).map(s => s.grade!.score)
      if (scores.length > 0) gradedScores[t.id] = Math.max(...scores)
    })
  }
  const isCompleted = isStudent && allTopics.length > 0 && Object.keys(gradedScores).length === allTopics.length
  const avgScore = isCompleted
    ? Math.round(Object.values(gradedScores).reduce((a, b) => a + b, 0) / allTopics.length)
    : 0
        ...
        <div className="p-6">
          ...
          {isStudent && allTopics.length > 0 && (
            <div className="mt-5 pt-5 border-t border-gray-100">
              {isCompleted ? (
                <div className="flex items-center justify-between gap-4 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-emerald-800">Курс пройден!</p>
                      <p className="text-xs text-emerald-600">Средний балл: {avgScore}/100 · {allTopics.length} тем</p>
                    </div>
                  </div>
                  <Link
                    to={`/courses/${id}/certificate`}
                    className="shrink-0 inline-flex items-center gap-1.5 bg-emerald-600 text-white text-xs font-semibold px-4 py-2 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
